# Emulo Proof v1 operator runbook

Emulo Proof v1 is an unexecuted benchmark methodology. At this point no public result exists. The committed example is synthetic, non-scored, and non-comparable. It is a packaging example, not evidence that either condition performs better.

## Frozen claim boundary

The scored release is a complete-system comparison: the selected agent host as configured, once cold and once with the frozen Emulo profile installed. It is not a comparison of underlying models. The contract is:

- Emulo `v0.3.7`, commit `5f4008b0c0df40dcadb92c8fd1ba4dcf3aee40d0`.
...
- Every cell is a **clean-host cold start** with host persistent context absent. Emulo is installed only inside the Emulo condition's isolated home.
...
```powershell
$RunRoot = 'D:\emulo-proof-runs\v1-live'
...
```powershell
python -c "from proof.fixtures import seal_fixture; seal_fixture(r'D:\private-fixtures\work-primary', r'D:\emulo-proof-runs\v1-live', 'work-primary')"
```
...
```powershell
python -m proof.cli validate --manifest 'D:\emulo-proof-runs\v1-live\manifest.json'
```
...
```powershell
python -m proof.cli prepare-cell --manifest 'D:\emulo-proof-runs\v1-live\manifest.json' --cell-id '<frozen cell ID>' --run-root 'D:\emulo-proof-runs\v1-live'
```

Execution is intentionally noisy and gated:

```powershell
python -m proof.cli execute-cell --manifest 'D:\emulo-proof-runs\v1-live\manifest.json' --cell-id '<frozen cell ID>' --run-root 'D:\emulo-proof-runs\v1-live' --execute --approval '<exact manifest_sha256 approved by Ohad>'
```
...
```powershell
python -m proof.cli evaluate --record 'D:\emulo-proof-runs\v1-live\record.json' --policy 'D:\emulo-proof-runs\v1-live\policy.json'
```
...
```powershell
python -m proof.cli package --manifest 'D:\emulo-proof-runs\v1-live\manifest.json' --records 'D:\emulo-proof-runs\v1-live\public-records.json' --evidence-root 'D:\emulo-proof-runs\v1-live' --destination 'D:\emulo-proof-runs\v1-live\candidate-publication' --ship-approval '<exact evidence digest approved by Ohad>' --privacy-inputs 'D:\emulo-proof-runs\v1-live\privacy-inputs.json' --manual-review-approved
```
