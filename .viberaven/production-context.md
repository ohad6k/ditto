...
- Boundary: `PAID_CHECKOUT_ENABLED=false`; `GOOGLE_CLIENT_ID=not-configured`; Cloudflare declares only `GITHUB_CLIENT_SECRET`, `POLAR_ACCESS_TOKEN`, and `POLAR_WEBHOOK_SECRET`. The Polar production webhook endpoint is enabled. No secret value was read, and no Google secret, customer, entitlement, device, generation, or payment was created.
- Danger: Authenticated paid continuity and billing behavior remain unproven in the live environment. The first 100% discount was limited through the next billing date rather than forever, so Polar requested card details. The checkout was aborted without entering payment data. Polar explicitly warns against production test purchases with real card details; the next verification must use a truly forever 100% discount or a private free product.
...
- Provider/MCP proof: Cloudflare listed version `c81f8a99-4c1b-4cf7-b872-d76e3a3e561c` after deploy and later secret-change versions without revealing values. A count-only D1 read returned one account, one GitHub identity, one browser session, and zero billing customers, entitlements, continuity devices, and continuity generations; the query wrote zero rows. Polar MCP created raw endpoint `ecfed609-ebc4-4814-9cdd-847c050365bb` at the exact production webhook URL with only the seven handled subscription events. After both Polar secret names were installed, an unsigned webhook returned `403`, checkout remained `503`, the endpoint was enabled, and its delivery list remained empty. A later Polar read verified country `IL`, logo and website configured, a payout account present, and checkout payments, subscription renewals, payouts, refunds, API access, and dashboard access all enabled. Customer, order, subscription, and payment counts remained zero. Version `37374377-f196-488d-ae15-179f172f2625` is the code rollback target.
- Open action: Correct the verification discount to 100% forever or use a private free recurring product, then obtain separate approval for another short checkout window. Proceed only if Polar requires no card, prove the genuine signed subscription lifecycle, capture the active controls at 390 px, run the complete synthetic continuity lifecycle, and disable checkout again. Google may remain hidden for the initial GitHub-only release.
