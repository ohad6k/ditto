...
- Danger: Authenticated paid continuity and billing behavior remain unproven in the live environment. Polar explicitly warns against production test purchases with real card details; production verification must use a free product or a 100% discount so no money changes hands.
...
- Open action: Create a bounded 100% production verification discount, obtain separate approval for a short checkout window, prove the genuine signed subscription lifecycle without a real charge, capture the active paid controls at 390 px, run the complete synthetic continuity lifecycle, and disable checkout again. Google may remain hidden for the initial GitHub-only release.
