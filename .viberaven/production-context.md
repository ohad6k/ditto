...
- Boundary: `PAID_CHECKOUT_ENABLED=false`; `GOOGLE_CLIENT_ID=not-configured`; the only declared production secret is `GITHUB_CLIENT_SECRET`. The Polar production webhook endpoint exists but is disabled until its signing secret is installed. No Polar token, webhook secret binding, Google secret, customer, entitlement, device, generation, or payment was created.
