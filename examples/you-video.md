...
- Inside a held world the phrases accumulate instead of cutting: captions slam-swap in place, a rapid list becomes one note card gaining lines, the host changes pose in place, the camera steps deeper each phase, and a kinetic line is a brief full-frame cut-away that returns to the same world.
  - Action: Render later phrases as phase layers on the world (caption swap, note lines, pose crossfade, stepped push with drift between steps, opaque slam cut-away) and never rebuild the frame within a beat; a source like a tweet enters once per beat and evolves, never re-enters.
