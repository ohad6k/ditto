# the unified system — where emulo fits

A common question: how does emulo sit alongside graph memory (Obsidian, llm-wiki),
...
```
  ...
  how you WORK  ────────►  emulo  <- this repo (~/.claude, ~/.codex .jsonl)
...
| **emulo** | raw session logs | how *you* decide, reject, get stuck |
| voice / persona | your writing | how you sound |
| soul.md | your working relationship with the agent | who the *agent* is |

emulo and soul.md are near mirror-images: **emulo profiles the human from the logs, soul.md lets
the agent articulate itself.** emulo is the one that reads the material you **never curated** —
your session logs are the you that just worked, not the you that you edited.

## the deliberate limit
emulo will not mine your notes, `CLAUDE.md`, or rules file as source. That would just reflect the
...
- emulo and soul.md pair naturally: run emulo for your side, soul.md for the agent's. Skill-mergers like agent-borg can absorb `you.md` into a bigger second-brain setup.
